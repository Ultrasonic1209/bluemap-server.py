...
class MapMeta(Base):
    ...
    map = Column(SMALLINT(5, unsigned=True), ForeignKey('bluemap_map.id'), primary_key=True, nullable=False)
    key = Column(VARCHAR(255), primary_key=True, nullable=False)
...
class MapTile(Base):
    ...
    map = Column(SMALLINT(5, unsigned=True), ForeignKey('bluemap_map.id'), primary_key=True, nullable=False)
    type = Column(SMALLINT(5, unsigned=True), ForeignKey('bluemap_map_tile_type.id'), primary_key=True, nullable=False)
